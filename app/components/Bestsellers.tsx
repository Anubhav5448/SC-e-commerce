...
export default function Bestsellers() {
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(
      ...
      { rootMargin: "400px 0px" } // start loading before the user reaches it
    );
  ...
  return (
    <section id="bestsellers" className="mx-auto max-w-7xl px-5 py-16 sm:px-8">
      ...
      <div className="grid grid-cols-2 gap-5 sm:grid-cols-3 lg:grid-cols-5 sm:gap-6">
        {visible.map((p) => {
          ...
          return (
            <Reveal
              key={p.id}
              delay={(Number(p.id.replace("p", "")) - 1) * 40}
            >
      ...
      {hasMore && (
        <div
          ...
          aria-hidden
        >
          <div className="h-8 w-8 animate-spin rounded-full border-2 border-ink/15 border-t-maroon" />
        </div>
      )}
    </section>
  );
